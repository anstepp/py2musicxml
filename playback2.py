...
if fourVoices:
    ...
    amp = 5000
    env = maketable(
        "curve", 1000, 0, 0, 2, 100, 1, 2, 200, 0.9, 2, 900, 0.9, 2, 1000, 0
    )
...
if stepwise:
    ...
    for item in noteList5:
        duration = item.dur * factor
        MMODALBAR(
            start, duration, amp, cpspch(item.pitch), 0.5, 0.5, 0, random.random()
        )
        start += duration
    ...
    for item in noteList6:
        duration = item.dur * factor
        STRUM2(
            start, duration, amp, item.pitch, random.random(), duration, random.random()
        )
        start += duration
    ...
    for item in noteList7:
        duration = item.dur * factor
        MMODALBAR(
            start, duration, amp, cpspch(item.pitch), 0.5, 0.5, 1, random.random()
        )
        start += duration
